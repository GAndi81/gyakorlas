...
function sortingDatas(sortingKey) {
    var tmp;
    var first;
    var second;
    for (var i = 0; i < jsonContent.length - 1; i++)
        for (var j = i + 1; j < jsonContent.length; j++)
            first = jsonContent[i][sortingKey];
    second = jsonContent[j][sortingKey];
    ...
    sortingDatas("FIFA ranking");

    console.log(jsonContent);

    //Összeállítom a táblát és kiíratom
    ...
    function createTable() {
        ...
        for (var i = 0; i < jsonContent.length; i++) {
            ...
            for (j in jsonContent[i]) {
                table += `<td> ${jsonContent[i][j]} </td>`;
            }
            // e fentivel tudunk sok elemet kiszedni ebből, a fentivel
            ...
            td > $ {
                jsonContent[i]["FIFA ranking"]
            } < /td> <
            td > $ {
                jsonContent[i].Group
            } < /td> <
            td > $ {
                jsonContent[i].Byline
            } < /td> <
            td > $ {
                jsonContent[i].Coach
            } < /td> < /
        ...
        table += '</tbody>';
        document.getElementById('#football tbody').innerHTML = table;
    }
    createTable();

    //ha FIFA ranking szeretném módosítani a keresést, akkor ezt fent találhatom.